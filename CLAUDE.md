...
Both are POSTed to the **`token_endpoint` from the registration file** (falling back to `<auth>/oauth/token`) with `grant_type=urn:aid:agent-identity`. `aid-token.sh` also accepts `--api-key / -k` to add an `X-Api-Key` header for servers that gate the token endpoint. The response is cached under `tokens/` keyed by **`SHA256(auth_url|credential_type)`** prefix, with a 60-second expiry buffer and scope-aware invalidation. Requesting a different `--scope` or a different `--credential-type` bypasses cache and gets a fresh token — the `|credential_type` in the key ensures JWT and api_key requests for the same auth server live in separate cache entries.

**Credential type (F007)**: an optional `requested_credential_type` form parameter selects `access_token` (RS256 JWT, default) or `api_key` (opaque bearer). The response carries a `credential_type` field indicating what was issued. The value is in the standard `access_token` response field regardless of type — keep that uniform; do not introduce a separate `credential` field.
